...
$('.close_box_material').click(function(){
...
var times = setInterval(function(){
    $.ajax({
        ...
        headers: {
            ...
            clearInterval(times);
            $('#_list_tickets').html('');
            let i =1; tk_arrival = tk_complete = tk_finalize = 0;
            arrTickets = dt.data;
            dt.data.forEach(element => {
                if(element.study_status == 4){
                    tk_arrival = tk_arrival +1;
                    $('#_list_tickets').append(
                        '<tr><td>'+ i +'</td><td>'+ element.ticket_title+'</td><td>'+element.order_workplace+'</td><td>'+element.schedule_date+'</td><td><button type="button" class="btn btn-primary" onclick="ViewTicket(' + element.id + ')">View</button></td></tr>'
                    );
                }
                if(element.study_status == 8){
                    tk_complete = tk_complete + 1;
                }
                if(element.study_status == 11){
                    tk_finalize = tk_finalize + 1;
                }
                i++;
            });
            $('#ticket_arrival_number').text(tk_arrival);
            $('#ticket_completed_number').text(tk_complete);
            $('#ticket_finalize_number').text(tk_finalize);
        ...
        $.ajax({
            ...
            Authorization: 'Bearer '+ tk,
               ...
               // console.log(dt.data);
                ...
                $('.process_ticket').css('display','block');
            },error: function(err){
                console.log(err);
            }
        });
    ...
    $('#orderMaterial').click(function(){
          ...
          $('.order_material').css('display','block');
      })
